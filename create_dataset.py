...
def create_data_subset(out_path, labels, files, augment=False, aug_angle=5, all_tumor_labels=True):
    ...
    for i in tqdm(range(nf)):
        if files[i][files[i].rfind("/") + 1:] == 'BraTS20_Training_355_seg.nii':
            continue
        ...
        for aug_num, image in enumerate(images):
            ...
            file_name = join(out_path, f'subj_{subj_id}_{aug_num}.nii')
...
def create_dataset(args):
    ...
    file_ids = [i.split(os.sep)[-1].replace(".nii","").replace("_seg","") for i in files]
